...
async def create_house_method(request: Request, db: Session = Depends(get_db), account: Users = Depends(get_if_user)):
    ...
    if form.is_valid():
        try:
            ...
            return responses.RedirectResponse(f"/details/{house.id}", status_code=status.HTTP_302_FOUND)
...
@router.get("/houses")
def show_your_houses(request: Request, account: Users = Depends(get_if_user), db: Session = Depends(get_db)):
    if account:
